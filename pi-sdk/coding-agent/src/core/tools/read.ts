...
export function createReadToolDefinition(
	...
	return {
		name: "read",
		label: "read",
		description: `读取文件内容。支持文本文件和图片（jpg、png、gif、webp）。图片会作为附件发送。对于文本文件，输出会被截断到最后 ${DEFAULT_MAX_LINES} 行或 ${DEFAULT_MAX_BYTES / 1024}KB（以先达到者为准）。对大文件使用 offset/limit 参数。当需要完整文件时，继续使用 offset 直到读完。`,
