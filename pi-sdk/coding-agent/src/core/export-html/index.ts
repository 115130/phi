...
import { APP_NAME, getExportTemplateDir, isBunBinary } from "../../config.ts";
...
function loadExportTemplates(): {
	...
	if (isBunBinary && (globalThis as any).__EMBEDDED_EXPORT_TEMPLATES__) {
		const embeds = (globalThis as any).__EMBEDDED_EXPORT_TEMPLATES__ as Record<string, string>;
		return {
			template: embeds.html,
			templateCss: embeds.css,
			templateJs: embeds.js,
			markedJs: embeds.markedJs,
			hljsJs: embeds.hljsJs,
		};
	}
	const templateDir = getExportTemplateDir();
	return {
		template: readFileSync(join(templateDir, "template.html"), "utf-8"),
		templateCss: readFileSync(join(templateDir, "template.css"), "utf-8"),
		templateJs: readFileSync(join(templateDir, "template.js"), "utf-8"),
		markedJs: readFileSync(join(templateDir, "vendor", "marked.min.js"), "utf-8"),
		hljsJs: readFileSync(join(templateDir, "vendor", "highlight.min.js"), "utf-8"),
	};
}

function generateHtml(sessionData: SessionData, themeName?: string): string {
	const { template, templateCss, templateJs, markedJs, hljsJs } = loadExportTemplates();
