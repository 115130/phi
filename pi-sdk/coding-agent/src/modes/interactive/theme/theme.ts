...
import { getCustomThemesDir, getThemesDir, isBunBinary } from "../../../config.ts";
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		if (isBunBinary && (globalThis as any).__EMBEDDED_THEMES__) {
			BUILTIN_THEMES = (globalThis as any).__EMBEDDED_THEMES__ as Record<string, ThemeJson>;
		} else {
...
function parseThemeJsonContent(label: string, content: string): ThemeJson {
	...
	try {
		...
		throw new Error(`解析主题 ${label} 失败: ${error}`);
	}
